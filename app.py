...
# --- 備忘録（シグナル定義・数値基準） ---
    with st.sidebar.expander("📖 シグナル判断の数値基準"):
        ...
        * **MACD上抜けシグナル**
        　条件: 当日の MACD線 > 0 かつ 前日の MACD線 <= 0。
        　意味: マイナス圏（下落トレンド）で推移していたMACD線が、ゼロラインを下から上へ突き抜けた状態です。相場の地合いが好転し、本格的な上昇トレンドに移行したことを示唆する強い買いサインとして機能します。
        * **RSI売られすぎ**
        ...
        * **MACD上抜けシグナル**
        　条件: 当日の MACD線 < 0 かつ 前日の MACD線 >= 0。
        　意味: プラス圏（上昇トレンド）で推移していたMACD線が、ゼロラインを上から下へ突き抜けた状態です。上昇エネルギーが尽き、本格的な下落トレンドに移行したことを示唆する強い売りサインとして警戒されます。
        * **RSI買われすぎ**
