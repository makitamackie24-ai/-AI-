...
st.write("過去の株価データからテクニカル指標を計算し、機械学習（ランダムフォレスト）を用いて翌日および1週間後（5営業日後）の株価が上昇する確率と、各価格を予測します。")
...
if 'analysis_results' in st.session_state:
    ...
    sort_basis = st.radio(
        "ランキングの基準を選択:",
        ("翌日予測ベース", "1週間後予測ベース", "翌日変動幅(高値-安値)予測ベース"),
        horizontal=True
    )
    ...
    with col_up:
        if sort_basis == "翌日変動幅(高値-安値)予測ベース":
        ...
        for i in range(min(10, len(results_sorted))):
            ...
            with st.container(border=True):
                ...
                if sort_basis == "翌日予測ベース":
                    ...
                    st.markdown(f"<h3 style='color: orange; margin-top: -10px; margin-bottom: 0px;'>予測変動幅: ¥{stock['pred_diff']:,.0f}</h3>", unsafe_allow_html=True)
                    st.markdown(f"<p style='color: #2e7d32; font-weight: bold;'>翌日上昇確率: {stock['score']:.1f}%</p>", unsafe_allow_html=True)
