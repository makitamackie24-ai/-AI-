...
with col_setting1:
    top_n = st.slider(f"解析対象とする売買代金上位の企業数を選択 (最大{stock_count}社)", min_value=10, max_value=stock_count, value=300, step=10, help="全銘柄から直近の売買代金が多い企業を自動選出し、AI解析の対象を絞ります。数を減らすと計算時間が短縮されます。")
with col_setting2:
...
if 'analysis_results' in st.session_state:
    ...
    with st.expander("💡 【備忘録】表示されるテクニカルサインの読み解き方と活用法"):
        ...
        *   **RSI過熱(70%超) / +2σ超過:** 強い上昇トレンドの最中はこれらが出続けます。「買うのをやめる」のではなく、**「そろそろ勢いが止まるかもしれないから、利益確定を早めにしよう」というエグジット警戒情報**として使います。
        ...
        ---
        **⚠️ 【重要】朝イチの「ギャップダウン」による想定外の損失を防ぐ対策**
        
        W指値（利確と損切りの同時注文）で、「〇〇円以下になったら【成行】で売る」という設定をしていると、翌朝に悪材料などで株価が想定以上に低く始まった（ギャップダウンした）場合、**その低い初値で即座に売却されてしまい、設定した損切りラインを大きく超える大損**になるリスクがあります（スリッページ）。
        
        これを防ぐための3つの対策をご紹介します。
        
        **1. 執行条件を「成行」ではなく「指値」にする（推奨）**
        「950円以下になったら【指値945円】で売る」といったように、成行ではなく少し下の指値で指定します。これにより、900円などの極端な安値で寄り付いた際に売らされるのを防げます。
        
        **2. 寄り付き直後は逆指値を外す**
        朝の9:00〜9:15頃まではダマシのギャップダウンが起きやすいため、この時間は注文を無効にするか、相場が落ち着いてから手動で逆指値を入れます。
        
        **3. 決算発表前はポジションを縮小する**
        ギャップダウンの最大の原因は「決算」などのイベントです。AIも突発的な決算内容は予測できないため、保有銘柄の決算発表日前には、W指値に頼らず一旦すべて売却しておくのが最も安全です。
        """)
